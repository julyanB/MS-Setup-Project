...
using EmployeeManagementService.Application.ExternalClients;
...
using EmployeeManagementService.Infrastructure.Http;
using EmployeeManagementService.Infrastructure.Identity;
...
using EmployeeManagementService.Infrastructure.Services;
using Hangfire;
...
public static class InfrastructureConfiguration
{
        ...
        IConfiguration configuration)
    {
        ...
        // Fall back to the shared application database when no dedicated connection is configured.
    ...
    private static IServiceCollection AddDatabase(
        ...
        IConfiguration configuration)
    {
        services.AddDbContext<EmployeeManagementServiceDbContext>(options => options
            .UseSqlServer(
                configuration.GetConnectionString("DefaultConnection"),
                b => b.MigrationsAssembly(typeof(EmployeeManagementServiceDbContext)
                    .Assembly.FullName)));

        services.AddScoped<IEmployeeManagementServiceDbContext>(provider =>
            provider.GetRequiredService<EmployeeManagementServiceDbContext>());
        ...
        IConfiguration configuration)
    {
        services
            ...
            .AddEntityFrameworkStores<EmployeeManagementServiceDbContext>();
        ...
        IConfiguration configuration)
    {
        ...
        services
            .AddRefitClient<ICoreServiceApi>()
            .ConfigureHttpClient(c =>
                c.BaseAddress = new Uri(configuration["ExternalApis:CoreService"]!))
